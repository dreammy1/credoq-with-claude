...
class Field_Event extends Field_Type {
	...
	public function get_settings_schema() : array {
		return [
			[
				'key'     => 'label',
				'type'    => 'text',
				'label'   => __( 'Field label', 'credoq-events' ),
				'default' => __( 'Event Registration', 'credoq-events' ),
			],
			[
				'key'     => 'max_tickets',
				'type'    => 'number',
				'label'   => __( 'Max tickets per booking (0 = unlimited)', 'credoq-events' ),
				'default' => 10,
			],
			[
				'key'     => 'show_capacity',
				'type'    => 'checkbox',
				'label'   => __( 'Show remaining capacity to customers', 'credoq-events' ),
				'default' => true,
			],
			[
				'key'     => 'required',
				'type'    => 'checkbox',
				'label'   => __( 'Required', 'credoq-events' ),
				'default' => true,
			],
		];
	}
	...
	public function validate( $value, array $field_config, array $submission ) {
		...
		try {
			...
			return new \WP_Error(
				'event_validate_exception',
				sprintf(
					/* translators: %s = exception message */
					__( 'Event Registration validation error: %s', 'credoq-events' ),
					$e->getMessage()
				)
			);
		}
	...
	private function validate_selections( $value, array $field_config, array $submission ) {
		...
		if ( ! is_array( $selections ) ) {
			return new \WP_Error( 'invalid_event', __( 'Please select at least one event.', 'credoq-events' ) );
		}

		// Normalise legacy single-event
		if ( isset( $selections['event_id'] ) ) {
	...
	public function credit_cost( $value, array $field_config, array $submission ) : int {
		try {
			if ( '' === $value || ! class_exists( '\\CredoqEvents\\Event_Repository' ) ) {
				return 0;
			}
			$selections = json_decode( $value, true );
			if ( ! is_array( $selections ) ) {
			...
			foreach ( $selections as $sel ) {
				...
				if ( $total <= 0 ) {
					continue; // nothing to pay for, credit not needed
				}
	...
	private function decide_payment( array $selections, array $submission = array() ) : array {
		if ( isset( $this->credit_decision ) ) {
			return $this->credit_decision;
		}
	...
	private function seat_overrides( array $submission ) : array {
		if ( null !== $this->seat_overrides_cache ) {
			return $this->seat_overrides_cache;
		}
	...
	private function resolve_selection_payment( object $event, float $total_price, array $decision ) : string {
		...
		if ( ! empty( $event->credit_deduct_enabled ) && $decision['use_credit'] ) {
			return 'credit';
		}
		if ( (int) $event->wc_product_id > 0 ) {
			return 'wc';
		}
		return 'unpayable';
	}
	...
	private function build_wc_contribution( $value, array $submission = array() ) : array {
		...
		foreach ( $selections as $sel ) {
			...
			if ( 'wc' !== $this->resolve_selection_payment( $event, $total, $decision ) ) {
				continue;
			}
	...
	public function on_submission( int $submission_id, $value, array $field_config, array $submission_payload ) {
		...
		try {
			...
			return new \WP_Error(
				'event_submission_exception',
				sprintf(
					/* translators: %s = exception message */
					__( 'Event Registration submission error: %s', 'credoq-events' ),
					$e->getMessage()
				)
			);
		}
